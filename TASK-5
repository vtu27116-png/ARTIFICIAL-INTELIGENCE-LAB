...
#intializingpheromne present at the paths to the cities
...
combine_feature = np.zeros(5)     #intializingcombine_feature array to zero
cum_prob = np.zeros(5)            #intializingcummulative probability array to zeros
...
dist_cost = np.zeros((m,1))             #intializingtotal_distance_of_tour with zero 
...
pheromne[int(rute_opt[i,j])-1,int(rute_opt[i,j+1])-1] = pheromne[int(rute_opt[i,j])-1,int(rute_opt[i,j+1])-1] + dt   
            ...
            #delta_distance will be more with min_disti.e adding more weight to that route  peromne
